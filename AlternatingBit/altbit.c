...
int ComputeChecksum(struct pkt packet)
{
  ...
  for (int i = 0; i < 20; i++) {
    checksum += packet.payload[i]; // sum all characters in the packet
  }
  checksum += packet.acknum; // add acknowledgement number to checksum
  checksum += packet.seqnum; // add sequence number to checksum
  return checksum;
}
...
void A_output(struct msg message)
{
  ...
  if ( windowcount < WINDOWSIZE) {
    ...
    starttimer(A, RTT); // after sending, starts timer for packet and waits for ACK to be received
  ...
  if (!IsCorrupted(packet)) {
    ...
    if (packet.acknum != A_nextseqnum && windowcount >= WINDOWSIZE) {    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
      ...
      /***** 1. FILL IN CODE  What else needs to be done when an ACK arrives
       ...
       stoptimer(A); // stops timer once ACK is received
    }
    else
      if (TRACE > 0)
...
void A_timerinterrupt(void)
{
  ...
  starttimer(A, RTT); // if ACK is not received within timer duration, resend packet and restart timer
  packets_resent++;
}       
...
void B_input(struct pkt packet)
{
  ...
  if  ( (!IsCorrupted(packet))  && (packet.seqnum == expectedseqnum) ) {
    ...
    /***** 3. FILL IN CODE  What ACK number should be sent if the packet
     ...
     sendpkt.acknum = (expectedseqnum + 1) % 2; // sends ACK of the last correctly received packet
  }
...
void B_init(void)
{
  expectedseqnum = 0; // changed the expected sequence number to 0 instead of 1
  B_nextseqnum = 1;
}
